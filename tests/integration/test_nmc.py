...
def test_nmc_returns_finite_scalar(setup_linear):
    nmc = NestedMonteCarloEIG(
        likelihood=setup_linear.likelihood, prior=setup_linear.gaussian_prior
    )
...
def test_laplace_error_grows_with_lambda():
    ...
    for lam in [0.0, 0.5, 1.0]:
        ...
        eig_nmc = NestedMonteCarloEIG(
            likelihood=s.likelihood, prior=s.gaussian_prior
        ).estimate(key, n_outer=4000, n_inner=4000)
